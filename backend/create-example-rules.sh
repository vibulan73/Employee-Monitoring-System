#!/bin/bash
# Script to create example login rules for all four types
# Make sure backend is running on http://localhost:8080

echo "Creating example login rules..."
echo ""

# Note: ALL_DAYS "Unrestricted Access" already exists as default rule

# 2. ALL_DAYS_WITH_TIME - Business Hours (9 AM to 5 PM every day)
...
# 3. DAY_ANY_TIME - Weekdays Only (Monday-Friday, any time)
...
# 4. CUSTOM - Flexible Schedule (Different times for different days)
...
# 5. CUSTOM - Night Shift (Example with overnight shift split)
